...
function f17() {
    let out = '';
    for (let key in a17) {
        if (a16[key]['name'] > 30) out += a16[key]['name'] + ' ';
...
function f18() {
    ...
    let out = '';
    // if (input in a18) out = a18[input] + ' ';  // Вариант № 1
    for (let key in a18) {
        if (input == key) out += a18[key];     //  Вариант № 2
    }
